        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            <div class="overflow-hidden">
                <div class="px-6 py-5 border-b">
                    <h2 class="text-2xl font-bold text-gray-800">Cập nhật khuyến mãi</h2>
                </div>

                <div class="p-6">
                    <form action="{{ route('promotions.update', $promotion) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="mt-2 w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 shadow-sm focus:outline-none">{{ old('description', $promotion->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="file" name="image" id="image" accept="image/*"
                                class="mt-2 w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 shadow-sm">
                            @error('image')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="discount_percent" id="discount_percent"
                                class="mt-2 w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 shadow-sm focus:outline-none"
                                value="{{ old('discount_percent', $promotion->discount_percent) }}" required min="0" max="100">
                        ...
                        <div>
                            <div class="flex items-center mb-2">
                                <input type="checkbox" name="is_permanent_checkbox" id="is_permanent_checkbox" 
                                    ...
                                    {{ old('is_permanent_checkbox', $promotion->isPermanent()) ? 'checked' : '' }}
                        ...
                        <div>
                            ...
                            <select name="status" id="status"
                                class="mt-2 w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 shadow-sm bg-white focus:outline-none">
                        ...
                        <div>
                            ...
                            <select name="booking_type" id="booking_type"
                                class="mt-2 w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 shadow-sm bg-white focus:outline-none">
                                <option value="all" {{ old('booking_type', $promotion->booking_type) == 'all' ? 'selected' : '' }}>Tất cả đơn đặt</option>
                                <option value="single" {{ old('booking_type', $promotion->booking_type) == 'single' ? 'selected' : '' }}>Đơn đặt theo buổi</option>
                                <option value="subscription" {{ old('booking_type', $promotion->booking_type) == 'subscription' ? 'selected' : '' }}>Đơn đặt định kỳ</option>
                            </select>
                            @error('booking_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
