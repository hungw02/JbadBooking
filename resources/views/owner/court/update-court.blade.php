...
@section('title', 'Cập nhật thông tin sân')
        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            <div class="overflow-hidden">
                <div class="px-6 py-5 border-b">
                    <h2 class="text-2xl font-bold text-gray-800">Cập nhật sân</h2>
                </div>

                <div class="p-6">
                    <form action="{{ route('courts.update', $court) }}" method="POST" class="space-y-5" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="available" {{ old('status', $court->status) == 'available' ? 'selected' : '' }}>
                                    Sẵn sàng
                                </option>
                                <option value="maintenance" {{ old('status', $court->status) == 'maintenance' ? 'selected' : '' }}>
                                    Bảo trì
                                </option>
                            </select>
                            @error('status')
                        ...
                        <div id="maintenance_dates" class="space-y-4 {{ old('status', $court->status) == 'maintenance' ? '' : 'hidden' }}">
                            <div>
                                ...
                                <input type="datetime-local" name="maintenance_start_date" id="maintenance_start_date"
                                    class="mt-2 w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 shadow-sm focus:outline-none"
                                    value="{{ old('maintenance_start_date', $court->maintenance_start_date ? \Carbon\Carbon::parse($court->maintenance_start_date)->format('Y-m-d\TH:i') : '') }}">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="maintenance_end_date" id="maintenance_end_date"
                                    class="mt-2 w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 shadow-sm focus:outline-none"
                                    value="{{ old('maintenance_end_date', $court->maintenance_end_date ? \Carbon\Carbon::parse($court->maintenance_end_date)->format('Y-m-d\TH:i') : '') }}">
                        ...
                        <div>
                            ...
                            @if($court->image)
                                <div class="mt-2 mb-3">
                                    <img src="{{ asset($court->image) }}" alt="{{ $court->name }}" class="h-40 w-auto object-cover rounded-lg border">
                                    <p class="mt-1 text-sm text-gray-500">Hình ảnh hiện tại</p>
                                </div>
                            @endif
                            <input type="file" name="image" id="image" 
                                class="mt-2 w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 shadow-sm focus:outline-none"
                                accept="image/*">
                            <p class="mt-1 text-sm text-gray-500">Đăng tải hình ảnh mới để thay thế (nếu muốn)</p>
                            @error('image')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
