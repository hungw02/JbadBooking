        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            <div class="overflow-hidden">
                <div class="px-6 py-5 border-b">
                    <h2 class="text-2xl font-bold text-gray-800">Cập nhật giá thuê</h2>
                </div>
                ...
                <div class="p-6">
                    <form action="{{ route('court-rates.update', $courtRate) }}" method="POST" class="space-y-5">
                        @csrf
                        @method('PUT')
                                ...
                                @foreach($dayMapping as $dayValue)
                                    <div class="flex items-center">
                                        <input type="checkbox" name="days_of_week[]" value="{{ $dayValue }}" 
                                            ...
                                            {{ in_array($dayValue, old('days_of_week', [$courtRate->day_of_week])) ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="time" name="start_time" id="start_time" min="05:00" max="24:00"
                                ...
                                value="{{ old('start_time', \Carbon\Carbon::parse($courtRate->start_time)->format('H:i')) }}" required>
                            @error('start_time')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="time" name="end_time" id="end_time" min="05:00" max="24:00"
                                class="mt-2 w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 shadow-sm focus:outline-none"
                                value="{{ old('end_time', \Carbon\Carbon::parse($courtRate->end_time)->format('H:i')) }}" required>
                            @error('end_time')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="price_per_hour" id="price_per_hour"
                                class="mt-2 w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 shadow-sm focus:outline-none"
                                value="{{ old('price_per_hour', $courtRate->price_per_hour) }}" required min="0">
