...
@section('title', 'Đổi sân')

@section('content')
<div class="max-w-7xl mx-auto p-5">
    <div class="mb-4 flex items-center">
        <a href="{{ route('booking.detail', $booking->id) }}" class="text-blue-600 hover:underline flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="bg-blue-600 p-4">
            <h1 class="text-white text-xl font-bold">Đổi sân - Lịch đặt #{{ $booking->id }}</h1>
        </div>
        ...
        <div class="p-6">
            <div class="bg-blue-50 rounded-lg p-4 border border-blue-100 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="font-medium text-lg">{{ \Carbon\Carbon::parse($booking->start_time)->format('d/m/Y') }}</span>
                    </div>
                    <div class="flex flex-col">
            ...
            @if(count($availableCourts) > 0)
                <form action="{{ route('booking.change.submit', $booking->id) }}" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        ...
                        <div class="bg-white border rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                            ...
                            @endif
                            <div class="p-4">
                                <h3 class="font-semibold text-lg mb-2">Sân {{ $court->name }}</h3>
                ...
                <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-lg p-4 mb-6">
                    <div class="flex">
                        ...
                        <div>
                            ...
                            <p>Không có sân nào khác còn trống trong khung giờ này. Vui lòng thử lại sau hoặc chọn khung giờ khác.</p>
                        </div>
                    </div>
                </div>
                
                <div class="flex justify-center">
                    <a href="{{ route('booking.detail', $booking->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded transition">
    ...
    <div class="mt-8">
        <h2 class="text-lg font-semibold text-gray-700 mb-4">
            <i class="fa-solid fa-circle-info mr-2"></i>Lưu ý khi đổi sân
        </h2>
        <div class="bg-white p-4 rounded-lg shadow">
            <ul class="list-disc pl-5 space-y-2 text-gray-700">
                <li>Việc đổi sân chỉ thay đổi địa điểm thi đấu, không thay đổi thời gian và ngày đặt.</li>
                <li>Nếu sân mới có giá khác với sân cũ, chúng tôi sẽ liên hệ để xác nhận phần chênh lệch.</li>
                <li>Bạn có thể đổi sân tối đa 3 lần cho mỗi lịch đặt.</li>
                <li>Việc đổi sân phải được thực hiện trước thời gian đặt ít nhất 3 giờ.</li>
            </ul>
        </div>
    </div>
</div>
